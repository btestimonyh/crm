    ...
    return (
        <div className="w-full flex items-center justify-start px-8 max-sm:px-2 gap-4 mb-4 max-sm:justify-between">
            ...
            <div className={`relative border-[1px] border-gray-500 rounded-xl p-2 cursor-pointer px-4`} onClick={() => setIsOpen(!isOpen)}>
                ...
                {isOpen && <div className="absolute max-sm:top-0 max-sm:flex-col max-sm:h-screen max-sm:fixed max-sm:w-full max-lg:left-[-100px] max-sm:left-0 flex gap-2 left-0 bg-[#121921] border-gray-500 z-10 rounded-xl flex items-center justify-center shadow-xl" onClick={(e) => e.stopPropagation()}>
                    <div className="absolute top-4 right-4 text-[30px] sm:hidden" onClick={()=> setIsOpen(false)}><IoMdClose/></div>
                    <div className="text-center border-r-2 px-4 flex flex-col gap-2 w-max self-start max-sm:self-center mt-8 max-sm:border-r-[0px] max-sm: ">
